...
let match = (curLineIdx: number, lineStartIdxs: number[], context: string): t_tableMatchResult => {
    /**
     ...
     */
    let line: string = util.getLine(lineStartIdxs[curLineIdx], context);
    line = util.extractFixes(line, '|').source;

    /**
     * after removed unneccassary pipes get the inner pipe count
     * and detect how many columns must be
     */
    let pipeCount: number = util.ccount(line, '|');
    let columns: number = pipeCount + 1;

    /**
     * get the next line string and remove unneccassary outer pipes
     */
    let nextLine: string = util.getLine(lineStartIdxs[curLineIdx + 1], context);
    nextLine = util.extractFixes(nextLine, '|').source;
    ...
    .map((str: string) => {
        let {
            ...
            left,
            right
        } = util.extractFixes(str.trim(), ':');
        let isValid: boolean = util.isConsistOf(source.trim(), '-');
    ...
    while (rowIdx < lineStartIdxs.length) {
        let row: string = util.getLine(lineStartIdxs[rowIdx], context);

        if (util.ccount(row, '|') == 0) {
            break;
        }
...
let extract = (curLineIdx: number, lineStartIdxs: number[], context: string,
rowRange: number, columnCount: number, textAligns: t_textAlign[]): Element => {
     ...
     */
    let headstr: string = util.getLine(lineStartIdxs[curLineIdx], context);
    headstr = util.extractFixes(headstr, '|').source;
    ...
    for (let i: number = 0; i < rowRange; i++) {
        let rowstr: string = util.getLine(lineStartIdxs[rowIdx + i], context);
        rowstr = util.extractFixes(rowstr, '|').source;
